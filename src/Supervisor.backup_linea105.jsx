...
export default function Supervisor() {
  ...
  const coincideDia = (diaItem, diaFiltro) => {
    if (!diaFiltro || diaFiltro === 'TODOS') return true;
    if (!diaItem) return false;
    const dNorm = diaItem.toString().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
    const fNorm = diaFiltro.toString().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
    if (dNorm === fNorm) return true;
    if (fNorm.startsWith(dNorm) || dNorm.startsWith(fNorm)) return true;
    // Mapeo abreviado clásico LUN, MAR, MIE, JUE, VIE, SAB
    const mapa = {
      'lun': 'lunes', 'mar': 'martes', 'mie': 'miercoles',
      'jue': 'jueves', 'vie': 'viernes', 'sab': 'sabado'
    };
    if (mapa[fNorm] && dNorm.startsWith(mapa[fNorm])) return true;
    if (mapa[dNorm] && fNorm.startsWith(mapa[dNorm])) return true;
    return false;
  };

  const comerciosDia = comerciosPreventista.filter(function(c) {
    return coincideDia(c.dia_visita, filtroDia);
  });
      ...
      <div style={{ padding: '10px 20px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center', borderBottom: '1px solid #1e293b', backgroundColor: '#0c1322' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '4px' }}>
          {['TODOS', 'LUN', 'MAR', 'MIE', 'JUE', 'VIE', 'SAB'].map(d => (
            <button key={d} onClick={() => { setFiltroDia(d); setSecuenciaParadas([]); setComercioActivo(null); }} style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', border: 'none', cursor: 'pointer', backgroundColor: filtroDia === d ? '#2563eb' : '#1e293b', color: filtroDia === d ? '#fff' : '#94a3b8' }}>
              {d}
            </button>
          ))}
